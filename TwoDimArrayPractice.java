	   ...
	   public void setValues( int value, int row )
	   {
	    ...
	    // Write your code here:
		   ...
		   for(int k = 0; k < intArray.length; k++)
		   {
			   if(k == row)
			   {
				   for(int column = 0; column < intArray[k].length; column++)
				   {
					   intArray[k][column] = value;
					   animate(row, column);
				   }
			   }
		   }
	   ...
	   */
	   public int countFound( int value )
	   {
	    ...
	    // Write your code here:
		   ...
		   for(int row = 0; row < intArray.length; row++)
		   {
			   for(int c = 0; c < intArray[row].length; c++)
			   {
				   if(intArray[row][c] == value)
				   {
					   valueFound++;
				   }
				   
				   animate(row, c, valueFound);
			   }
		   }
		   

	     return valueFound; // replace this line with your return statement
	   }
